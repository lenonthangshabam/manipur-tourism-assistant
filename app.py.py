...
def transcribe_audio(uploaded_audio, language: str):
    if not SPEECH_RECOGNITION_AVAILABLE:
        return None, (
            "Voice transcription is not installed on this deployment. "
            "The text and Manipuri features are still available."
        )
...
with st.sidebar:
    ...
    if not SPEECH_RECOGNITION_AVAILABLE:
        st.warning(
            "Voice transcription is unavailable in this deployment, but all text features remain active."
        )
...
if not SPEECH_RECOGNITION_AVAILABLE:
    st.info(
        "Voice recording is available, but speech-to-text is not installed in this deployment. "
        "Use text input for English, Hindi, or Manipuri while the deployment dependency is being fixed."
    )
